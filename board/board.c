...
Board *B_update(Board *board) {
  ...
  for (int i = 0; i < board->height; i++) {
    for (int j = 0; j < board->width; j++) {
      ...
      else if (alive_neighbours[i][j] == 2 && _is_alive(board->cell[i][j]))
        B_set_alive(new_board, i, j);
      else
...
static int _count_alive_neighbours(Board *board, int i, int j) {
  ...
  if (board->version == CIRCULAR) {
    ...
    for (int k = 0; k < 8; k++) {
      if (_is_alive(neighbours_circle[k]))
        count++;
    }
    ...
    if (i != 0) {
      if (j != 0)
        if (_is_alive(board->cell[i - 1][j - 1]))
          count++;
      if (_is_alive(board->cell[i - 1][j]))
        count++;
      if (j != width - 1)
        if (_is_alive(board->cell[i - 1][j + 1]))
          count++;
